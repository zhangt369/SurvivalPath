...
data <- read.csv("D:/Code/R/SurvivalPath0225/Survivaldata/generator/mid3.csv",)
...
mk = df2newick(df,innerlabel = T,survivaltime = 3,period=12)

mytree <- read.tree(text=mk$newick)
sizetree <- read.tree(text=mk$size)
survivaltree <- read.tree(text=mk$survival)
survivalratetree <- read.tree(text=mk$survivalrate)
...
ggtree(mytree1, color="black",linetype=1,size=1.,ladderize = T, )+
  theme_tree2() +
  geom_text2(aes(label=label),hjust=0.6, vjust=-0.5 ,size=2.0)+
  geom_text2(aes(label=paste(size,mytree1@data$survival,mytree1@data$survivalrate,sep = "/")),hjust=0.6, vjust=-1.65 ,size=2.4)+
  #geom_point2(aes(shape=isTip, color=isTip), size=mytree1@data$os/40)+
  geom_point2(aes(shape=isTip, color=isTip), size=mytree1@data$size%/%100+1)+
  labs(size= "Nitrogen",
       x = "TimePoints",
       y = "Survival",
       title = "Survival Tree")
