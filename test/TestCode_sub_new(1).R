...
#Converting time points data into time slices data
dataset = timedivision(X2021data,"ID","Date",period = 90,left_interval = 0.5,right_interval=0.5)
table(dataset$time_slice)

#extract data and arrange by time slice
time <- list()
...
result <- survivalpath(df$time,df$status,df$timeslicedata,df$tspatientid,time_slices=2, degreeofcorrelation=0.7, rates=365*1)

#export time-slice dataset with node annotation
write.csv(result$df, file = "output.csv", row.names = FALSE)
