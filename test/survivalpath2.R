...
#'@usage survivalpath()
#'@param time (list by time_slice), survival time
#'@param status  (list by time_slice),survival status
#'@param  timeslicedata (list by time_slice), clinical data
#'@param tspatientid (list by time_slice), patients ID
...
survivalpath <- function(time,status,timeslicedata,tspatientid,time_slices,treatments=NULL,num_categories=2,
                         p.value=0.05,minsample = 15,degreeofcorrelation=0.7) {
  l_time <- length(time)
  ...
  for(i in 1:l_time){
    ...
    print(i)
    print(dim(ts_data))
    print(dim(result$source))
    result$source <- rbind(result$source,ts_data)
  }
  result$treatmentsnum <- dim(treatments[[1]])[2]
  ...
  for (time_slice in 1:time_slices){
    ...
    patinetsindex <- alldata$data$class==time_slice
    current_ts_data <- alldata$data[patinetsindex,]

    current_time <- current_ts_data["time"]

    #Full sample status of current time node
    current_status <- current_ts_data["status"]

    #Full sample id of current time node
    current_id <- current_ts_data["ID"]

    #All sample variables for the current time node: data frames
    current_variables <- current_ts_data[,(5+ alldata$treatmentsnum):dim(current_ts_data)[2]]
    ...
    for (i_id in 1:brnum) {
      ...
      current_names <- isbranch(current_ts_b_id,minsample) #Empty or stop
      ...
      mainvar <- selectvariables(current_ts_b_time,current_ts_b_status,cox,degreeofcorrelation=0.7)
  ...
  result$tree <-  df2newick(result$data,innerlabel = T, period=365)
...
kmselectfeature <- function(time,status,variables,num_categories,p.value=0.05){
...
selectvariables <- function(time,status,coxvar,degreeofcorrelation=0.7){
  ...
  while (FLAG) {
    coxvar <- pairwise(time, status,coxvar,degreeofcorrelation=0.7)
  }
...
pairwise <- function(time, status,coxvar,degreeofcorrelation=0.7){
